...
export type SendArgs = {
  ...
  replyTo?: string;
  bcc?: string;
  // Override the display name in the From header while keeping the configured
  // deliverable address. Used for direct-to-mom emails so the inbox shows the
  // buyer's name. Spoofing the buyer's address is intentionally not supported —
  // DKIM/SPF protect deliverability.
...
// Splits "Name <addr@example.com>" into { name, address }. Bare addresses
// return { name: undefined, address }. Used by fromDisplayName override.
function parseFromAddress(from: string): { name?: string; address: string } {
  const m = from.match(/^\s*(.+?)\s*<([^>]+)>\s*$/);
  if (m) return { name: m[1], address: m[2] };
...
export async function sendEmail(args: SendArgs): Promise<SendResult> {
  ...
  try {
    const fromHeader = args.fromDisplayName
      ? `${args.fromDisplayName} <${parseFromAddress(FROM).address}>`
      : FROM;
